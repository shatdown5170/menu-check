...
9. ★판독 불가: 사진의 가장자리에서 잘렸거나, 빛 반사/빛번짐, 심한 흐림(저화질)으로 글자를 읽을 수 없는 부분은 절대 추측하지 마. 그 메뉴·가격은 dishes 에 넣지 말고 unreadable 에 기록해 — 어느 사진(image, 1부터)의 어느 위치(where)인지, 사유(reason: 잘림/빛번짐/흐림), 그리고 가능하면 그 영역의 bbox 를 사진 크기 대비 0~1 비율 [x, y, w, h](좌상단 기준)로. bbox 를 모르면 그 항목에서 bbox 키를 빼. 같은 사진에서 같은 영역(예: 우측 가격 열 전체)은 행마다 쪼개지 말고 하나의 항목으로 묶어서 bbox 도 그 영역 전체를 덮게 보고해. 명확히 읽히는 부분만 dishes 로 출력해. 전부 잘 보이면 unreadable 은 빈 배열 [].`;
